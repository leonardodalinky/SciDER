...
class MetaChainLogger:
    ...
    def _warp_args(self, args_json: str):
    ...
    def _wrap_title(self, title: str, color: str | None = None):
    ...
    def info(self, *args: str, color: str = "white", title: str = "INFO", **kwargs):
        # console = Console()
    ...
    def lprint(self, *args: str, color: str = "white", title: str = "", **kwargs):
        if not self.debug:
            return
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        message = "\n".join(map(str, args))
...
class LoggerManager:
    ...
    @classmethod
    def get_logger(cls) -> MetaChainLogger:
        return cls.get_instance()._logger  # type: ignore
...
if DEFAULT_LOG:
    ...
    LoggerManager.get_logger().info(
        ...
        LoggerManager.get_logger().log_path or "None",
        "...",
        title="Log Path",
        color="light_cyan3",
    )
