...
Usage:
    from scider.core import openhands_import  # Must be before openhands imports
...
# OpenHands is intentionally optional in SciDER. If it's not enabled, this module
# becomes a no-op so importing it cannot mutate sys.path unexpectedly.
_ENABLE_OPENHANDS = __import__("os").getenv("SCIDER_ENABLE_OPENHANDS", "").strip().lower() in {
    "1",
    "true",
    "yes",
    "y",
...
if not _ENABLE_OPENHANDS:
    ...
    _SCIDER_ROOT = Path(__file__).parent.parent.parent
    _OPENHANDS_SDK_PATH = _SCIDER_ROOT / "software-agent-sdk" / "openhands-sdk"
    _OPENHANDS_TOOLS_PATH = _SCIDER_ROOT / "software-agent-sdk" / "openhands-tools"
    ...
    # Calculate project root (this file is in scider/core/)
    _SCIDER_ROOT = Path(__file__).parent.parent.parent
    _OPENHANDS_SDK_PATH = _SCIDER_ROOT / "software-agent-sdk" / "openhands-sdk"
    _OPENHANDS_TOOLS_PATH = _SCIDER_ROOT / "software-agent-sdk" / "openhands-tools"
